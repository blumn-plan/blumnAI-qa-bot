...
clone 해두고, 그 안에서 아래 6개를 우리 정책 레포의 .blumnAI-qa-bot/ 아래로
복사해줘:
...
- 새로 만들 파일 .blumnAI-qa-bot/version — 확장자 없음. 내용은 딱 한 줄로
  코어 레포 CHANGELOG.md 최상단 버전 (예: "v0.1.0"). 이 파일은 코어 자동 업데이트
  알림 배너가 원격 CHANGELOG 와 비교할 때 씀 — 반드시 있어야 배너 동작.
...
| `examples/sample-policy-repo/.blumnAI-qa-bot/version` **파일 1개** | `.blumnAI-qa-bot/` 바로 밑에 |
