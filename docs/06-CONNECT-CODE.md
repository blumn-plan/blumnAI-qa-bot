...
> 🚨 **뭔가 안 되면 답변 상단 배지 툴팁부터 확인** — 이 기능의 모든 실패 케이스는 배지에 원인이 뜹니다.
> - 🔍 초록 = 정상 · ⚠️ 주황 = 실패 (툴팁에 원인 힌트) · 📄 회색 = 아직 미설정
> - 주황이면 배지에 마우스 올려서 툴팁 문구 확인 → [트러블슈팅 표](#트러블슈팅) 에서 색·문구별 조치.
> - 매칭 0건이면 배지 옆 "GitHub 에서 직접 검색해보기 ↗" 링크 클릭해서 실제 인덱싱 확인.
...
projects:
  - id: ○○_v1
    ...
    code_paths:                             # (선택) 관심 경로 glob. 비우면 레포 전체.
      - "src/pages/**/*.tsx"                # ⚠️ **1개만** 넣으세요. GitHub Search API 는 다중 path
                                            #    qualifier 를 AND 로 처리해서 여러 개 넣으면
                                            #    "동시에 모든 경로에 있는 파일" = 매칭 불가능.
                                            #    2개 이상 넣으면 봇이 자동으로 무시하고 레포 전체 검색.
    code_search_hint: ""                    # (선택) 매 검색에 함께 붙일 힌트 키워드. 자주 나오는
                                            #    영문 심볼 몇 개. 예: "Dashboard Campaign Filter".
                                            #    한글 질문일 때 매칭 확률 상승.
...
## ⭐ 매칭 확률을 확 올리는 팁 — 코드 파일 맨 위 한글 주석 한 줄

GitHub Search Code API 는 리터럴 매칭이라 한글 질문을 한글 코드 주석과 그대로 매치합니다. 화면-단위 컴포넌트 파일 맨 위에 아래 한 줄만 넣어두면 매칭 확률이 극적으로 상승:
...
**소요 시간**: 15개 화면이면 약 15분. 파일명·구조 안 바꾸고 주석 한 줄만.

**효과**:
- 사용자가 "메시지 통계 화면의 발송률은 어떻게 계산?" 라고 물으면 봇이 즉시 `MessageStatsPage.tsx` 를 찾아 답변에 인용.
- 워커가 한글 질문 토큰 (`메시지`, `통계`) 을 검색어 최상위 우선순위로 두므로, 한글 주석 한 줄이면 top 검색 결과에 등장.
- 컴포넌트명 (영문) 기억 안 나도 정책 문서·화면명 그대로 질문하면 코드까지 안내됨.

이 방식이 `code_search_hint` 채우기·`code_paths` 좁히기·`extractCodeSymbols` 자동 추출보다 훨씬 안정적이에요. 새 팀 셋업 시 첫 15분 투자로 두고두고 답변 품질 확보.
