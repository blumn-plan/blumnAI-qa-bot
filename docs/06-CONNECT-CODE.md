...
| 증상 | 원인·해결 |
|---|---|
| 답변에 코드 스니펫이 전혀 안 나옴 | ① `config.yml` 의 `code_repo` 미채움 ② PAT 이 코드 레포 접근권 없음 ③ 질문 키워드가 검색에 안 걸림 → 더 구체적인 질문 (예: 파일명·함수명 언급) |
...
| 코드 스니펫 fetch 실패해도 답변은 나옴 | 정상 동작 — worker 는 코드 fetch 실패 시 정책만으로 답. 콘솔 warn 만 남음 |
