...
- [🚀 왜 이 봇이 필요한가 — 핵심 장점 4가지](#-왜-이-봇이-필요한가--핵심-장점-4가지)
- [💡 이렇게 활용하세요 — 실전 시나리오 8가지](#-이렇게-활용하세요--실전-시나리오-8가지)
- [🟢 세팅하기 — 처음 깔 때](#-세팅하기--처음-깔-때-필수)
- [필요할 때 쓰는 3가지 프롬프트](#필요할-때-쓰는-3가지-프롬프트)
- [🔀 여러 문서를 함께 참고해서 답변받고 싶다면](#-여러-문서를-함께-참고해서-답변받고-싶다면)
- [🔎 더 알아보기](#-더-알아보기) (한계·문서 목록·폴더 구조)
...
### 3. 세팅하기 마스터 프롬프트 ← ⭐ 여기가 핵심 액션

**이 프롬프트 하나가 사실상 세팅의 전부** 예요. 아래 접이식 박스를 클릭해서 **회색 코드 박스 전체** 를 복사한 뒤 **Claude Code 창에 붙여넣으면** 됩니다.

**왜 이걸 붙여넣나요?** — 그동안 사람이 30-60분 걸리던 작업 (레포 생성 · 파일 복사 · config 작성 · 시크릿 등록 · Worker 배포 · GitHub Pages 활성화 · 첫 테스트) 을 **AI 에게 위임** 해서 15-30분 안에 끝냅니다.

**붙여넣으면 뭐가 일어나나요?** — Claude Code 가 아래 순서로 진행하고, 매 스텝마다 사용자에게 "다음 진행 OK?" 확인을 받아요:

| 단계 | AI 가 하는 일 (🤖) | 본인이 할 일 (👤) |
|---|---|---|
| 1. 자동 발견 | GitHub 스캔해서 정책·화면·코드 레포·프로젝트 폴더 찾기 → 표로 확인 요청 | "네" 확인만 |
| 2. 파일 얹기 | 팀 정책 레포 안에 `.blumnAI-qa-bot/` 폴더 + `config.yml` 자동 생성 | — |
| 3. 시크릿 등록 | `wrangler secret put` 명령어 실행 | 발급받은 API key · PAT 를 터미널에 붙여넣기 (딱 이 순간만) |
| 4. Worker 배포 | `wrangler deploy` 실행 → URL 획득 | — |
| 5. Pages 활성화 | 어느 화면 눌러야 하는지 안내 | GitHub Settings 에서 클릭 몇 번 |
| 6. 첫 테스트 | 5가지 시나리오 순서대로 시연 | 봇에 질문 던져보기 |
...
| 🟠 업데이트하기 | 새 버전 알림이 왔을 때 (봇 상단 노란 배너) | 1주 ~ 1달에 한 번 (초기 · 활발한 개선기) → 안정되면 분기 1회 |
