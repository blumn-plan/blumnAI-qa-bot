...
**정책·화면설계서·서비스 코드 기반으로 AI 가 자동응답을 해주고, 개선 필요하면 기획자에게 자동으로 전달하는 사내 QA 챗봇.**
...
### ⭐ 1. 미리 준비할 4가지 — 이거 없으면 프롬프트가 도중에 멈춰요

> [!IMPORTANT]
> **먼저 아래 4가지가 손에 있어야 세팅 프롬프트가 매끄럽게 흘러갑니다.** 특히 1번 (Anthropic API key) 은 IT기획팀 지라 대기 시간이 있으니 **가장 먼저 요청** 해두세요.
...
| 2 | **정책·화면설계서 GitHub 레포** — 봇을 얹을 홈 (기획 레포) | 이미 있으면 스킵 · 없으면 GitHub 에서 신규 생성 (Private 권장). 정확한 레포 이름을 몰라도 세팅 프롬프트에서 `[모름]` 표시하면 AI 가 조직 내 후보 자동 검색해줍니다 | 이미 있으면 0분 · 없으면 3분 |
| 3 | **Cloudflare 계정** — Worker 무료 호스팅 | https://dash.cloudflare.com/sign-up 에서 이메일 + 비밀번호로 가입 → 인증 메일 클릭 → 카드 등록 화면은 **Skip** (무료 tier 라 카드 불필요) | 3분 (👤 본인이 직접) |
| 4 | **Node.js 18+** | 미리 확인 안 해도 OK — AI 가 첫 스텝에서 자동으로 확인해줍니다 | 없으면 +5분 |

**전체 준비 소요 시간**: 대부분 IT기획팀 응답을 기다리는 시간입니다.

### 2. 아래 [세팅하기 마스터 프롬프트](#-3-세팅하기-마스터-프롬프트--여기가-핵심-액션) 에 채워야 할 값들 안내

**아래 [세팅하기 마스터 프롬프트](#-3-세팅하기-마스터-프롬프트--여기가-핵심-액션) 영역에 채워야 할 값들에 대한 안내입니다.** 정확히 몰라도 OK — 프롬프트 안에 `[모름]` 이라고 적으면 AI 가 GitHub 을 자동 스캔해서 후보를 찾아 확인 요청해줍니다:
...
> [!IMPORTANT]
> **이 프롬프트 하나가 사실상 세팅의 전부예요.** 아래 접이식 박스를 클릭 → **회색 코드 박스 전체** 를 복사 → **Claude Code 창에 붙여넣으면** 끝. 나머지는 AI 가 확인받으면서 자동 진행.
...
봇 화면 좌측 상단 **[🌐 전체 정책 종합해서 답변]** 버튼을 클릭하면 됩니다.

- 특정 문서를 선택할 필요가 **없어져요**
- 프로젝트 안의 **모든 정책·화면설계서를 종합해서** 답변합니다
- 예: *"결제 정책과 알림 정책이 얽혀 있는데 이 케이스는?"* → 봇이 관련 정책 여러 개를 함께 참고해서 답

**언제 개별 문서 모드로 돌아가나?**
좌측 목록에서 정책 문서를 클릭하면 전체 모드가 자동으로 해제되고 그 문서 기준으로 돌아갑니다.

**설정은 필요 없나요?**
필요 없습니다. 예전에는 `bot.include_all_docs: true` 설정이 필요했지만, 지금은 화면 버튼 하나로 켜고 끌 수 있어요.

**주의**: 프로젝트 안 문서가 30개+ 로 많으면 첫 질문 응답이 느려지거나 (전체 문서 전송) 봇이 핵심을 놓칠 수 있어요. 이럴 땐 개별 문서 모드로 하나 골라서 물어보는 편이 답변 품질이 더 좋습니다.
